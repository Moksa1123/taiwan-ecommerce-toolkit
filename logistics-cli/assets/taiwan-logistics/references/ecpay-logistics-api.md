...
>
> 舊版本這裡把 `2030` 寫成「已交寄」、`2063` 寫成「配達完成」、`2073`／`2074` 寫成「退貨中／退貨完成」，
> 與綠界官方代碼表不符（`2073` 其實是「包裹配達取件門市」），已更正。
...
> 依綠界附錄「交易訊息代碼一覽表」（developers.ecpay.com.tw/7426）。舊版本把 `10500001` 寫成「MerchantID 為必填」、
> `10500003` 寫成「CheckMacValue 錯誤」等，與官方不符，已更正。綠界註明代碼持續新增，完整清單在廠商後台「交易狀態代碼查詢」。
