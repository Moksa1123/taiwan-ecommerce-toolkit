...
| `bank_code_list` | 指定發卡銀行 | 3 | 銀行代碼，如台新 `812`。多組用 Array：`['812','822']` |
...
| 項目 | 優先 | 來源 | 備註 |
|---|---|---|---|
| 電子發票 API 端點與參數 | 中 | 電子發票手冊 v2.3（70 頁，已存 `_studies/sunpay/`）| 尚未解析 |
| 虛擬帳號／超商代碼條碼／超商取貨的專屬欄位 | 中 | 金流手冊 §4.3.1.3–4.3.1.5 | 共同參數與信用卡已完成 |
| 請款 / 退款 API 的請求欄位 | 中 | 金流手冊 §4.4–4.5 | 端點已確認 |
| 物流貨態代碼 `StoreType` 對照表 | 中 | 金流手冊 §物流通知 | 併入 `taiwan-logistics/data/status-codes.csv` |
| 統一錯誤碼表 | 中 | — | ⚠️ 手冊未提供獨立錯誤碼章節，僅有各 API 的 `pay_result`；已全數收錄於 §6 |
