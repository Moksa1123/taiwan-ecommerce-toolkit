...
### 退款 API 的 `code`（手冊 4.6.2）

退款結果以 JSON 即時回傳，`code` 與交易通知、查詢的代碼**又是另一套**（`12`、`13` 意義都不同）：
...
查詢（4.4.2）與請款（4.5.2）的外層 `code` 只有 `00`（請求成功）與 `99`（查詢：未知錯誤；請款：請求失敗）；
查詢的業務狀態在 `result` 裡的狀態欄位（上表 06–15）。
