...
> PayNow 發票**沒有公開的錯誤代碼表**。官方 SOAP 規格（PayNow_EInvoice_v1.5）的回應只帶 `ErrorMsg` 文字欄位，
> 沒有代碼；REST（JWT）版的公開頁面也未列錯誤碼。請以 `ErrorMsg` 內容判斷並完整記錄，需要代碼表請洽
> einvoice@paynow.com.tw。
>
> 舊版本這裡列了自行「推估」的 `I000`～`I303` 代碼，沒有任何出處，已刪除，以免被當成真實代碼寫進程式。
